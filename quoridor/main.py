...
def play_game(
    ...
    while True:
        ...
        if "my-player" in game_state and game_state["my-player"]:
            ...
            if new_player != player and verbose and game_num > 0:
                print(
                    f"ℹ️  Player assignment changed: You are now Player {new_player}\n"
                )
            player = new_player

        game = game_class(
            game_state["state"], game_state["status"], game_state["player"], player
        )
                    ...
                    if verbose:
                        if hasattr(move, "__iter__") and not isinstance(move, str):
                            if len(move) > 0 and move[0] == "M":
                                ...
                                print(
                                    f"   Placing {move[3]} wall at ({move[1]}, {move[2]})"
                                )
                            else:
                                print(f"   Move: {move}")
                        else:
        ...
        if verbose:
            ...
            if game_turn_times:
                ...
                print(
                    f"⏱️ Turn timing this game: avg {avg_ms:.1f} ms, max {max(game_turn_times):.1f} ms over {len(game_turn_times)} turns"
                )

        # Check if game ended with an error
        if not game_state.get("winner"):
            print("⚠️  Game ended with an error - skipping to next game")
            all_results.append(("error", player, None))
            continue
    ...
    if session_turn_times and verbose:
        ...
        print(
            f"⏱️ Overall solver timing: avg {avg_ms:.1f} ms, max {max(session_turn_times):.1f} ms across {len(session_turn_times)} turns"
        )
...
def manual_player_solver(game: QuoridorGame) -> List:
    ...
    while True:
        try:
            choice = input(
                "\nEnter your choice (number for pawn move, or 'W r c H/V' for wall): "
            ).strip()
...
def my_agent(game: QuoridorGame) -> List:
    ...
    search_depth = 2
    total_walls = (
        root_state["remaining_walls"]["1"] + root_state["remaining_walls"]["2"]
    )
    if total_walls <= 4:
        search_depth = 3
    ...
    def evaluate_state(state: Dict) -> float:
        ...
        wall_diff = (
            state["remaining_walls"][my_player] - state["remaining_walls"][opponent]
        )
        wall_penalty = (
            -2
            if state["remaining_walls"][my_player] == 0
            and state["remaining_walls"][opponent] > 0
            else 0
        )
    ...
    def candidate_wall_moves(state: Dict, player: str) -> List[Tuple[float, List]]:
        ...
        raw_walls = temp_game.get_valid_wall_moves(
            player, limit=None, strategy=WALL_STRATEGY
        )
        if not raw_walls:
            return []
    ...
    def minimax(
        state: Dict, player: str, depth: int, alpha: float, beta: float
    ) -> float:
        ...
        if my_turn:
            ...
            for mv in moves:
                ...
                value = max(
                    value,
                    minimax(child_state, opponent_of(player), depth - 1, alpha, beta),
                )
    ...
    if not root_moves:
        fallback = game.get_valid_pawn_moves() + game.get_valid_wall_moves(
            limit=10, strategy=WALL_STRATEGY
        )
    ...
    for move in root_moves:
        ...
        score = minimax(
            next_state, opponent, search_depth - 1, -float("inf"), float("inf")
        )
        if score > best_score:
            best_score = score
            best_move = move
...
def test_partial(game_state: Optional[Dict] = None) -> Dict:
    ...
    test_game = QuoridorGame(
        json.dumps(state_payload), "playing", current_player, current_player
    )
    ...
    updated_game = QuoridorGame(
        json.dumps(updated_state), "playing", next_player, next_player
    )
    print("\nBoard after move:")
...
def self_play(
    num_games: int = 1, max_moves: int = 200, verbose: bool = True
) -> List[Tuple[str, int]]:
    """
    ...
    for game_idx in range(num_games):
        ...
        if turn_times and verbose:
            ...
            print(
                f"⏱️ Timing: avg {avg_ms:.1f} ms, max {max(turn_times):.1f} ms over {len(turn_times)} turns"
            )
...
def human_vs_agent(human_player: str = "1", max_moves: int = 200, verbose: bool = True):
    ...
    for ply in range(max_moves):
        ...
        if winner:
            if verbose:
                end_game = QuoridorGame(
                    json.dumps(next_state), "complete", current_player, human_player
                )
                end_game.print_board()
                print(f"Winner: Player {winner}\n")
            break
    ...
    if agent_turn_times and verbose:
        ...
        print(
            f"⏱️ Agent timing: avg {avg_ms:.1f} ms, max {max(agent_turn_times):.1f} ms over {len(agent_turn_times)} turns"
        )


def test_full():
    STUDENT_TOKEN = "Matty"  # e.g., 'JOHN-DOE'
    SOLVER = my_agent
    MULTIPLAYER = True
    MATCH_ID = None
    NUM_GAMES = 3
...
if __name__ == "__main__":
    ...
    if args.play_agent:
        ...
        while True:
            ...
            while True:
                user_input = (
                    input("Press Enter for next move or type 'stop' to exit: ")
                    .strip()
                    .lower()
                )
                if user_input == "":
